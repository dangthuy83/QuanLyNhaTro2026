...
- Ngày đến hạn kỳ N lấy `HopDong.NgayThanhToanHangThang` trong tháng N+1; nếu tháng ngắn hơn thì dùng ngày cuối tháng.
  - Triển khai REVIEW-017 snapshot kết quả vào `HoaDon.NgayDenHan` khi chốt hóa đơn. Công nợ, nhắc nợ và Excel đọc snapshot này, không tính động từ hợp đồng hiện tại.
  - Cho phép sửa `NgayThanhToanHangThang` kể cả khi hợp đồng đã có dữ liệu nghiệp vụ, nhưng giá trị mới chỉ áp dụng cho hóa đơn tạo sau lần sửa; hóa đơn cũ giữ nguyên ngày đến hạn. Hợp đồng mới khi chuyển phòng kế thừa ngày thanh toán của hợp đồng cũ.
  - Apply-once chỉ backfill hóa đơn chưa có snapshot từ ngày thanh toán hiện tại của hợp đồng và có blocker trước DDL. DB vận hành lúc áp REVIEW-017 có `HopDong=0`, `HoaDon=0`, nên không có lịch sử thực tế bị suy diễn.
...
- Hợp đồng tương lai dùng trạng thái `ChoHieuLuc`; chỉ chuyển `DangHieuLuc` khi đến ngày bắt đầu và không chồng kỳ.
  ...
  - Triển khai REVIEW-007 không tin `PhongId`, `TrangThai` hoặc giá gốc từ request Edit. Bản gốc được khóa/tải lại trong transaction; hợp đồng đã có dữ liệu nghiệp vụ chỉ cho sửa `GhiChu` và, theo quyết định REVIEW-017, `NgayThanhToanHangThang` cho các hóa đơn tương lai. Danh sách khách được giữ nguyên khi đã phát sinh dữ liệu; lịch sử hiệu lực nhân khẩu vẫn thuộc REVIEW-008.
