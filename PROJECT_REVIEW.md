...
#### REVIEW-028 - Chuẩn bị cutover và release

- **Trạng thái:** Pre-production rehearsal hoàn tất; migration 12 đã áp DB vận hành ngày
  17/07/2026. Chưa import dữ liệu mở sổ và chưa đổi/start bản production.
- Importer JSON validate-first đã có schema/template, dataset giả và từng mã chứng từ bắt buộc.
  `validate` là read-only; `apply` atomic toàn batch, xác nhận SHA-256 file nguồn và chống replay.
- Backup/restore khớp 21 bảng, 7 trigger, row count và journal 1..11. Migration 12 apply/rerun-safe
  trên restore tạm. Blocker thiếu chỉ số không tạo row; crash giữa batch rollback hoàn toàn; retry
  thành công và lần replay sau commit bị chặn.
- Hậu kiểm dataset rehearsal: 3 hợp đồng, 3 cư trú, 9 dịch vụ hợp đồng, 3 ledger `SoDuMoSo`,
  3 mốc chỉ số, cọc 3.700.000, công nợ 800.000; `HoaDon=ThanhToan=ThuChi=0`, snapshot công nợ
  chưa gắn, journal 1..12, continuity sạch và reconcile issue = 0.
- Release build/publish pass; artifact không mang config local/tool/schema. Browser QA cùng binary
  pass login và màn vận hành chính, Excel download, desktop/mobile, console sạch. HTTPS Production
  khởi động được nhưng Browser TLS/LAN còn phải kiểm tra với certificate production đã trust.
- Gate migration production đã pass: backup mới 55.244 byte, restore khớp 21/21 bảng, 7/7 trigger,
  exact row counts và journal; sequence 12/hash đúng manifest, journal liên tục 1..12 và tổng 78.
- **Còn thiếu / gate:** file nguồn thật và SHA, tổng kiểm soát từng nhóm, approval import thật,
  đường dẫn publish cũ, certificate/hostname LAN, service account, quyền thư mục
  `private-data/tenant-photos` và approval switch/start release.
- **Mức độ / loại:** Critical - production cutover; không được vượt approval gate.
