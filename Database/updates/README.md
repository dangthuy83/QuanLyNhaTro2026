...
- Rehearsal DB tạm: migration journal `1..12`; fresh schema có 25 bảng và 4 bảng mở sổ.
  DB vận hành đã áp đúng sequence 12 qua MigrationRunner sau backup/restore verify; journal `1..12`.
...
REVIEW-027/028 opening-balance cutover:

- `20260716_review027_opening_balances.sql` là migration sequence 12, tạo bốn bảng mở sổ và mở
  rộng ledger cọc; apply-once, blocker-first và chỉ được chạy qua MigrationRunner theo thứ tự.
- REVIEW-028 đã rehearsal apply/rerun trên DB restore tạm. Ngày 17/07/2026, DB vận hành được backup,
  restore verify khớp tuyệt đối rồi áp đúng sequence 12 qua MigrationRunner; hậu kiểm journal 1..12,
  bốn bảng mở sổ rỗng và dữ liệu nghiệp vụ không đổi. Không replay sequence 12.
- Sau migration 12, dùng `tools/OpeningBalanceImporter` theo README của tool. `validate` không ghi;
  `apply` chỉ dùng file thật đã duyệt, xác nhận SHA và atomic toàn đợt. Không dùng các file trong
  `samples/` cho production và không tạo `ThuCoc`/`ThanhToan` lịch sử giả.
